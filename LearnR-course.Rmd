...
R functions work the same way. In fact you have already seen a function.  Remember our first line:
...
The `dim` function displays the number of rows and columns in the data frame. In the code cell, you will see the code to call the `dim` function with the `gender_data` data frame as the argument. Run the code to see what `dim` returns.
...
**Q.** Use the `names` function with the `gender_data` data frame as an argument, to show the names of the data frame columns.
...
**Q.** Use the `str` function with the `gender_data` data frame as an argument, to show the structure of the `gender_data` data frame.
...
**Q.** Use the `summary` function with the `gender_data` data frame as an argument, to show descriptive statistics for each column.
...
**Q.** Use the `head` function with the `gender_data` data frame, to show the first six rows of the data frame. Then show the _last_ six rows with `tail`.
...
# This section of the code check gives an incorrect answer to the head section.
#check_output(state,
#  '1    Afghanistan   4\\.95450  19961015094[[:space:]]+     161\\.1380',
#  missing_msg='Did you display the value for "head(gender_data)"?')

# This gives a correct answer, but adding more to the regex arg causes an incorrect message.
check_output(state,
             'Afghanistan',
             missing_msg='Did you display the value for "head(gender_data)"?')
...
# This section of the code check gives an incorrect answer for the tail section.
#check_output(state,
...
**Q.** Try it in the code cell below. Use the `$` method to display all the values in the `mat_mort_ratio` column of `gender_data`.
...
**Q.** Make a new variable `health_exp_col` that points to the `health_exp_per_cap` column in the `gender_data` data frame.  Display the contents of your new `health_exp_col` variable.
...
Now use the code cell to show the last 6 rows in the data frame with `tail(gender_data)`. Look at the `health_exp_per_cap` column.

```{r dftail, exercise=TRUE}
# Load in data
gender_data <- read.csv('http://bit.ly/gender-stats-data')
...
Run `mean(health_exp_col)` in the code cell below and look at the result.
...
The code to use the `na.rm` option is in the script window. Run the code to see the result.
...
```{r na-hint}
If you have deleted the code, copy paste `mean(health_exp_col)` into the script window.
```
...
**Q.** First calculate the median of `health_exp_col` the default way, and show the result. It should show `NA`. Then calculate the median of `health_exp_col`, setting the option to drop the `NA` values.
...
To get the first value of `gdp`, we can use `gender_data$gdp[1]`. Notice the square brackets at the end, and inside the brackets, the index of the value we want. Try it in the code cell below.
...
**Q.** Divide the first value of the `gdp` column by the first value of the `population` column, to give the GDP per person, for the first row.

```{r gdp-pp, exercise=TRUE}
# Load in the data
gender_data <- read.csv('http://bit.ly/gender-stats-data')
...
Run the code in the cell below, to show the result of dividing the `gdp` column by the `population` column. Check that the first value is the same as the value you saw in the previous exercise.

```{r whole-divide, exercise=TRUE}
# Load in the data
gender_data <- read.csv('http://bit.ly/gender-stats-data')
...
**Q.** Calculate and show the total dollar health expenditure for every country, by multiplying the values in the `health_exp_per_cap` column by the corresponding values in the `population` column.

If you did it right, you will see lots of big numbers in scientific notation, like `5.271766e+09` ($5.271766 * 10^9$).

```{r total-dollar, exercise=TRUE}
# Load in te data
gender_data <- read.csv('http://bit.ly/gender-stats-data')
...
Run the code in the cell below, to show the histogram of the `mat_mort_ratio` values.
...
**Q.** Show the histogram of the values in the `health_exp_per_cap` column of `gender_data`.
...
**Q.** Make a new variable `mat_mort_col` that points to the `mat_mort_ratio` column of `gender_data`. Then show a histogram of the values of `mat_mort_col`.
...
The cell below contains the code to plot the `health_exp_per_cap` column of `gender_data` against the `mat_mort_ratio` column of `gender_data`. Run the code to see the result.
...
**Q.** Make a new variable `total_health_exp` with the total dollar health expenditure for every country. The variable should be the multiplication of the values in the `health_exp_per_cap` column by the corresponding values in the `population` column.
